...
/**
 * Floating cinema screen — NO Html for performance.
 * "SHOWREEL" text baked into screen texture.
 */
export default function ShowreelScreen({ scrollProgress }) {
  const groupRef = useRef()

  // Bake showreel text into a canvas texture
  const screenTex = useMemo(() => {
    ...
    ctx.fillStyle = 'rgba(108,124,255,0.7)'
  ...
  const frameMat = useMemo(() => new THREE.MeshBasicMaterial({ color: '#1A1D28' }), [])
